...
| Select Similar BBox Objects       | `Select_Similar_BBox_Objects.py`    | 以当前唯一选中的对象为参考，按 bounding box 最长边倍数范围选择尺寸相近对象，适合批量选择小零件、小标识、小装饰物。                                       |
| Keyframe Smoother                 | `Keyframe_Smoother.py`              | 复制当前选中的对象，并在复制体上平滑 Position / Rotation / Scale 动画曲线，用于处理烘焙动画、反求相机、跟踪数据等轻微抖动。                           |
| SimilarMesh Select Similar        | `SimilarMesh_SelectSimilar.py`      | 单选模式：以当前唯一选中的 Polygon Object 为参考，选择场景中几何特征相似的可见对象，不创建 Null，不移动层级。                                      |
| SimilarMesh Group Visible Meshes  | `SimilarMesh_GroupVisibleMeshes.py` | 分组模式：未选择时扫描全场景，选择多个对象时扫描选中对象及其子级，将相似可见对象整理到 `Auto_Similar_Groups` 下。                                   |

## SimilarMesh 工具说明

`SimilarMesh_SelectSimilar.py` 和 `SimilarMesh_GroupVisibleMeshes.py` 是从旧版 `SimilarMesh Grouper.py` 拆分出的两个独立脚本。旧脚本已不再使用。
...
### BBox Tiny Fragment Cleaner

使用 bounding box 尺寸检测场景中的微小碎片对象。
当对象 X/Y/Z 尺寸和体积低于设定阈值时，将其收集到备用组中，
方便用户确认后手动删除，避免误删有效模型。
